...
vector<t_annot_region*>* get_unique_regions_by_posn(vector<t_annot_region*>* all_regs, bool skip_duplicates)
{
	...
	for (int i_chr = 0; i_chr < restr_all_regs->chr_ids->size(); i_chr++)
	{
		int i_reg = 0;
		while(i_reg < restr_all_regs->regions_per_chrom[i_chr]->size())
		{
			int j_reg = i_reg;
			while (j_reg < restr_all_regs->regions_per_chrom[i_chr]->size() &&
