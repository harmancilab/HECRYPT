...
void load_dump_allele_switching_events_in_haplotypes(char* haplo_swtiching_list_fp, char* sample_ids_list_fp)
{
	...
	{
		per_hap_per_sample_switch_posns[i_hap] = new vector<int>*[train_sample_ids->size()];
	...
	for (int i_s = 0; i_s < train_sample_ids->size(); i_s++)
	{
		if (i_s == 3)
		{
			for (int i_hap = 0; i_hap < 2; i_hap++)
			{
				for (int i_switch = 0; i_switch < per_hap_per_sample_switch_posns[i_hap][i_s]->size(); i_switch++)
				{
					fprintf(stderr, "%d/%d :: %d/%d: %d\n", 
							i_s, i_hap, 
							i_switch, per_hap_per_sample_switch_posns[i_hap][i_s]->size(), 
